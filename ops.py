...
def binary_mask(shape):
    ...
    mask = tf.less_equal(samples, 0.5)
    return tf.cast(mask, tf.float32)


def salt_and_pepper_noise(x, salt_ratio=0.2, pepper_ratio=0.2):
    shape = x.get_shape()
    sample1 = tf.random_uniform(shape, minval=0.0, maxval=1.0)
    sample2 = tf.random_uniform(shape, minval=0.0, maxval=1.0)
    salt = tf.cast(tf.less_equal(sample1, salt_ratio), tf.float32)
    pepper = tf.cast(tf.less_equal(sample2, 1. - pepper_ratio), tf.float32)
    return tf.minimum(tf.maximum(x, salt), pepper)


def get_dim(x, index):
    return x.get_shape().as_list()[index]
